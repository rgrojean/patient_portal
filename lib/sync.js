...
// v3 primary PIS patient identifier system (Riverbend-namespaced).
var PRIMARY_PATIENT_SYSTEM = 'urn:riverbend:mrn';

function primaryIdentifierValue(p) {
  var ids = p && p.identifier;
  var i;
  if (!Array.isArray(ids)) {
    return null;
  }
  for (i = 0; i < ids.length; i++) {
    if (ids[i] && ids[i].system === PRIMARY_PATIENT_SYSTEM && ids[i].value) {
      return ids[i].value;
    }
  }
  return null;
...
async function upsertPatient(p) {
  ...
  if (!patientId) {
    console.error('missing_primary_identifier');
    throw new Error('missing_primary_identifier');
  }
  await db.query(
    `INSERT INTO portal_patients (patient_id, family, given, dob, gender, phone, email, addr_line1, city, state, zip)
     ...
     ON CONFLICT (patient_id) DO UPDATE SET
       ...
       zip = EXCLUDED.zip,
       updated_at = NOW()`,
    [patientId, p.family, p.given, p.dob, p.gender, p.phone, p.email,
...
module.exports = {
  ...
  PRIMARY_PATIENT_SYSTEM: PRIMARY_PATIENT_SYSTEM
};
